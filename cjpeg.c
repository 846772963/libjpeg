...
main (int argc, char **argv)
{
	...
	yuvData = (char *)malloc(image_size);
	fread(yuvData,image_size,1,infile);
	...
	for(; cinfo.next_scanline < cinfo.image_height; counter++)
	...
	{   
...

	 {
	      ...
	      int mem_size = buf_width[band];//buf_width[band];

	      pDst = (unsigned char *) buffer[band][0];

	      pSrc = (unsigned char *) rawData[band] + counter*buf_height[band] * buf_width[band];//buf_width[band];  //yuv.data[band]分别表示YUV起始地址
	...
	//max_line一般为16，外循环每次处理16行数据。
	 ...
	 counter = cinfo.image_height / 16 -1;
	...
	{	
...

	 {
		  ...
		  for(i=buf_height[band] -1; i >= 0; i--)  //处理每行数据

		  {

			   memcpy(pDst, pSrc + i *buf_width[band] , mem_size);
